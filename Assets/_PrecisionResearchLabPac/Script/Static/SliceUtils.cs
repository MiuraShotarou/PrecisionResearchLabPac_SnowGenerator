...
public static class SliceUtils
{
    ...
    /// 1次元配列用
    /// </summary>
    public static Array Slice<T>(this T[] arr, int start = 0, int stop = int.MinValue)
    {
        int len        = arr.Length;
    ...
    /// 2次元配列用
    /// </summary>
    public static Array Slice<T>(this T[,] arr,
        int rowStart = 0, int rowStop  = int.MinValue,
        int colStart = 0, int colStop  = int.MinValue)
    {
    ...
    /// <summary>
    /// 3次元配列用 channel省略時は全チャンネル、指定時は data[:, :, channel] に相当
    /// </summary>
    public static Array Slice<T>(this T[,,] arr,
        int rowStart = 0, int rowStop  = int.MinValue,
        int colStart = 0, int colStop  = int.MinValue,
        int channel  = int.MinValue)
    {
        int rows   = arr.GetLength(0);
        int cols   = arr.GetLength(1);
        int rStart = rowStart < 0 ? rows + rowStart : rowStart;
        ...
        for (int i = 0; i < rSize; i++)
        for (int j = 0; j < cSize; j++)
            result[i, j] = channel == int.MinValue
                ? arr[rStart + i, cStart + j, 0]
                : arr[rStart + i, cStart + j, channel];
        return result;
    }
    public static float[,,] SetChannel(this float[,,] arr, int channel, float[,] src)
    {
        int rows = arr.GetLength(0);
        int cols = arr.GetLength(1);
        for (int i = 0; i < rows; i++)
        for (int j = 0; j < cols; j++)
            arr[i, j, channel] = src[i, j];
        return arr;  // thisを返すことでチェーン可能
    }

    public static float[,,] SetChannel(this float[,,] arr, int channel, bool[,] src)
    {
        int rows = arr.GetLength(0);
        int cols = arr.GetLength(1);
        for (int i = 0; i < rows; i++)
        for (int j = 0; j < cols; j++)
            arr[i, j, channel] = src[i, j] ? 1f : 0f;
        return arr;
    }
}