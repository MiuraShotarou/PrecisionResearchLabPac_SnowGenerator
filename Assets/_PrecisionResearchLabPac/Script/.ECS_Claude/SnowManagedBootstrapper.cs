...
public class SnowManagedBootstrapper : MonoBehaviour
{
    ...
    [SerializeField] private int   _stepsPerFrame = 1;  // 1フレームに実行するステップ数
    ...
    /// <summary> SnowflakeCreate </summary>
    private IEnumerator SpawnRoutine()
    {
        ...
        for (int i = 0; i < _snowCount; i++)
        {
            ...
            em.AddComponentData(entity, new Unity.Transforms.LocalTransform
            {
                Position = new float3(
            ...
            // コルーチンでシミュレーションを実行しながらメッシュを更新
            yield return StartCoroutine(GrowAndUpdate(mf, _maxSteps));
    ...
    /// <summary>
    /// シミュレーションをstepsPerFrameずつ実行し、毎フレームメッシュを更新する
    /// </summary>
    private IEnumerator GrowAndUpdate(MeshFilter mf, int maxSteps)
    {
        var core = new SnowflakeCore(maxSteps, _scale);
        // core.RandomizeParams();

        int step = 0;
        while (step < maxSteps)
        {
            for (int s = 0; s < _stepsPerFrame && step < maxSteps; s++, step++)
                core.Step(); // SnowflakeCoreのStep()をpublicに変更する必要あり

            // 毎フレームメッシュを更新
            mf.mesh = core.BuildMesh();
...
//     private IEnumerator SpawnRoutine()
//     {
//         var world = World.DefaultGameObjectInjectionWorld; //Worldへの参照
...
//             });
//
//             entityManager.AddComponentData(entity, new SnowFallData
//             {
//                 FallSpeed  = _fallSpeed  * UnityEngine.Random.Range(0.7f, 1.3f),
//                 SwaySpeed  = UnityEngine.Random.Range(0.3f,  1.0f),
//                 SwayAmount = UnityEngine.Random.Range(0.01f, 0.05f),
...
//             entityManager.AddComponentObject(entity, new SnowMeshComponent
//             {
//                 Mesh         = mesh,
//                 MeshFilter   = mf,
//                 MeshRenderer = mr,
